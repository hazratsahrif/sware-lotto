...
class WithDarwHistoryView extends GetView<SettingsController> {
   WithDarwHistoryView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      AppBar(
        ...
        title: const Text('Withdraw History',style: TextStyle(color: Colors.black),),
      ),
      body: controller.userWithDrawResponse.value.withdraws!.length==0?
      Center(child: Text("No record found", style: TextStyle(color: Colors.black, fontSize: 20),))
          :RefreshIndicator(
        onRefresh: ()async{
          controller.getWithDrawApiResponse();
        },
            child: ListView.builder(
        itemCount: controller.userWithDrawList.length,
                      ...
                      children: [
                        Flexible(
                          child: RichText(text: TextSpan(
                              children: <TextSpan>[
                                TextSpan(text: controller.userWithDrawList[index].amount!.toString(), style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 30,
                        ...
                        Flexible(
                          child:
                                Text( controller.userWithDrawList[index].status==0?
                                      "Pending":
                                       "Approved",
                                    style: TextStyle(
                                    fontWeight: FontWeight.bold,
                                    fontSize: 16,
                                    color: controller.userWithDrawList[index].status==0? Colors.red:Colors.green
